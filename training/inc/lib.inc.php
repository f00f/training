...
function FirstWord($p_text) {
	$matches = array();
	preg_match('/^(.*?)[^A-Za-z0-9�������]/', $p_text, $matches);
	return $matches[1];
}
...
function UpdateFiles() {
	...
	$html = '<script>
		...
		Das n�chste Training ist am <strong>'.$nextTraining['wtag'].', '.date('d.m.', $nextTraining['datum']).' um '.$nextTraining['zeit'].'</strong> (Beckenzeit) in <strong>'.$nextTraining['ort'].'</strong><br />
